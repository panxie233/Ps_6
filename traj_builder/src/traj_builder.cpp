...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
    ...
    double y_start = start_pose.pose.position.y;
    double psi_start = convertPlanarQuat2Psi(start_pose.pose.orientation);
    double x_vel = vel.twist.twist.linear.x;
    double z_omega = vel.twist.twist.angular.z;
    double x_time = (x_vel/accel_max_);
    double z_time = (z_omega/alpha_max_);
    double max_time;
    if (x_time > z_time)
		max_time = x_time;
	else max_time = z_time;
	double x_dec = x_vel/max_time;
	double z_dec = z_omega/max_time;
	int npts = round(max_time/dt_);
    ...
    for (int i = 0; i < npts; i++) {
        ...
        x_vel -= x_dec*dt_;
        z_omega -= z_dec*dt_;
        des_state.twist.twist.linear.x = x_vel;
        des_state.twist.twist.angular.z = z_omega;
        x_start += x_vel*dt_*cos(psi_start);
        y_start += x_vel*dt_*sin(psi_start);
        psi_start += z_omega*dt_;
        des_state.pose.pose.position.x = x_start;
        des_state.pose.pose.position.y = y_start;
        des_state.pose.pose.orientation = convertPlanarPsi2Quaternion(psi_start); 
        vec_of_states.push_back(des_state);
    }
